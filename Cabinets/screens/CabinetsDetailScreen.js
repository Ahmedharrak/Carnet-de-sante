...
class CabinetsDetailScreen extends Component {

  constructor() {
    super();
    this.dbRef = firebase.firestore().collection('FavorisC');
    this.state = {
      name: '',
      image: '', 
      location: '', 
  ...
  componentDidMount() {
    const dbRef = firebase.firestore().collection('Cabinets').doc(this.props.route.params.cabinetskey)
    dbRef.get().then((res) => {
      if (res.exists) {
...
export default CabinetsDetailScreen;