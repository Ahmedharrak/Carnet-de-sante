...
class LaboratoiresDetailScreen extends Component {

  constructor() {
    super();
    this.dbRef = firebase.firestore().collection('FavorisL');
    this.state = {
      name: '',
      image: '', 
      location: '', 
  ...
  componentDidMount() {
    const dbRef = firebase.firestore().collection('Laboratoires').doc(this.props.route.params.laboratoireskey)
    dbRef.get().then((res) => {
      if (res.exists) {
        const Laboratoires = res.data();
        this.setState({
          key: res.id,
          name: Laboratoires.name,
          image: Laboratoires.image, location: Laboratoires.location, telephone: Laboratoires.telephone, 
          desc: Laboratoires.desc,
          isLoading: false
        });
      } else {
    ...
    if(this.state.name === ''){
     alert('Fill at least your name!')
      ...
      this.dbRef.add({
        ...
        this.props.navigation.navigate('Laboratoires')
      })
      .catch((err) => {
    ...
    return (
      <ScrollView style={styles.container}>
        ...
        <Text style={{ 
          ...
          <TextInput
          style={{ marginLeft:30,fontSize: 13,color : "white" }}
              placeholder={'Name'}
...
export default LaboratoiresDetailScreen;