...
class FavorisLDetailScreen extends Component {
  ...
  componentDidMount() {
    const dbRef = firebase.firestore().collection('FavorisL').doc(this.props.route.params.laboratoireskey)
    dbRef.get().then((res) => {
      if (res.exists) {
        ...
        this.setState({
          ...
          image: Laboratoires.image, location: Laboratoires.location, 
          telephone: Laboratoires.telephone, desc: Laboratoires.desc,
          isLoading: false
        });
      } else {
...
export default FavorisLDetailScreen;